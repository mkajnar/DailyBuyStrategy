...
class DailyBuyStrategy3_4_JPA(IStrategy):
    INTERFACE_VERSION = 3
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        # # Vypočet klouzavého součtu objemu pro poslední tři svíčky
        # if len(dataframe) >= 3:
        #     rolling_volume_sum = dataframe['volume'].rolling(window=3).sum()
        #     current_rolling_volume = rolling_volume_sum.iloc[-1]
        #     previous_rolling_volume = rolling_volume_sum.iloc[-4] if len(dataframe) > 3 else 0
        #
        #     # Kontrola, zda došlo k signifikantnímu nárůstu objemu
        #     if current_rolling_volume <= previous_rolling_volume * self.a_vol_coef.value:
        #         return None
