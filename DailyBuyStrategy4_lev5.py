...
class DailyBuyStrategy4_lev5(IStrategy):
    INTERFACE_VERSION = 3
    ...
    def calculate_mfi(self, dataframe: DataFrame) -> DataFrame:
        # Výpočet Money Flow Index (MFI)
        dataframe['mfi'] = ta.MFI(dataframe)
        return dataframe

    def calculate_rsi(self, dataframe, window=14):
        """ Výpočet RSI na základě zavíracích cen v pandas Series. """
    ...
    def add_qqe_to_dataframe(self, dataframe, window=14, sf=5, rsi_window=14):
        """ Výpočet QQE a přidání výsledků do zadaného DataFrame. """
        # Výpočet RSI
        dataframe['rsi'] = self.calculate_rsi(dataframe['close'], rsi_window)

        # Hladký RSI (Wilder's smoothing)
        dataframe['smooth_rsi'] = dataframe['rsi'].ewm(alpha=1 / window, adjust=False).mean()

        # Výpočet ATR of RSI
        dataframe['rsi_atr'] = abs(dataframe['smooth_rsi'].diff()).rolling(window=window, min_periods=1).mean()

        # Výpočet horního a dolního Fast QQE bandu
        dataframe['long_qqe'] = dataframe['smooth_rsi'] + sf * dataframe['rsi_atr']
        dataframe['short_qqe'] = dataframe['smooth_rsi'] - sf * dataframe['rsi_atr']
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe = self.calculate_mfi(dataframe=dataframe)
        return dataframe
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if not informative.empty:
            # Ensure informative is aligned with the main dataframe
            # This method requires timestamps as indexes in both dataframes
            informative = informative.reindex(dataframe.index, method='nearest')

            # Now compare the closes safely as they are aligned
    ...
    def confirm_trade_exit(self, pair: str, trade: Trade, order_type: str, amount: float,
        ...
        if 'stop_loss' in exit_reason:
            if len(self.get_dca_list(trade)) < 5:
                return False  # Pokračování v obchodování
            else:
                return True  # Ukončení obchodu po 10 pokusech
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        logging.info(f'{trade.pair}: CR {current_rate}, SL {self.get_mk_sl(trade)}')
        if current_rate * 1.01 > self.get_mk_sl(trade):
            return None
        ...
        if len(self.get_dca_list(trade)) < 3:
            ...
            logging.info(
                f"{current_time} - DCA triggered for {trade.pair}. Adjusting position with additional stake {trade.stake_amount * 3}")
