...
class DailyBuyStrategy5(IStrategy):
    INTERFACE_VERSION = 3
    ...
    dca_cooldown_candles = IntParameter(1, 30, default=5, space='buy', optimize=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe = self.prepare_indicators(dataframe)
        return dataframe

    def prepare_indicators(self, dataframe) -> DataFrame:
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        if dataframe.empty:
            ...
            dataframe = self.prepare_indicators(dataframe=dataframe)

        last_candle = dataframe.iloc[-1]
        last_index = dataframe.index[-1]

        if last_candle['resistance_signal'] == False:
            return None

        if (last_index % self.dca_cooldown_candles.value != 0):
            return None
